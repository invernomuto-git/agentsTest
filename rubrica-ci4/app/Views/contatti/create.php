...
<form method="post" action="<?= site_url('contatti/create') ?>">
    <?= csrf_field() ?>
    ...
    <div class="form-group">
        ...
        <input type="text" id="numero_telefono" name="numero_telefono"
               value="<?= esc($old['numero_telefono'] ?? '') ?>" required>
    </div>
    <div class="form-group">
        ...
        <select id="id_azienda" name="id_azienda">
            ...
            <?php foreach ($aziende as $az): ?>
                <option value="<?= esc($az['id']) ?>"
                    <?= (isset($old['id_azienda']) && (int) $old['id_azienda'] === (int) $az['id']) ? 'selected' : '' ?>>
                    <?= esc($az['nome']) ?>
                </option>
            <?php endforeach; ?>
        </select>
